...
class Buff_Unit(object):
    ...
    def setEmbedInfo(self, mask, layer):
        ...
        #         continue
        #     else:
        #         num_embed_recv[i] = recv_embed_idx_cpu[i].shape[0]
        
        # resize发送和接收变量
        if layer == 0:
            num_embed_send = self._layer_size[layer]
        else:
            num_embed_send = int(self._layer_size[layer]*1)
        # for i in range(size):
        #     if i == rank:
        #         continue
        #     else:
        #         node_num = self.f_send_cpu[layer][i].shape[0]
        #         self.f_send_cpu[layer][i].resize_([node_num+1, num_embed_send[i]])
        #         node_num = self.f_recv_cpu[layer][i].shape[0]
        #         self.f_recv_cpu[layer][i].resize_([node_num+1, num_embed_recv[i]])
        #         node_num = self.f_recv_gpu[layer][i].shape[0]
        #         self.f_recv_gpu[layer][i].resize_([node_num+1, num_embed_recv[i]])
        for i in range(size):
            if i == rank:
                continue
            else:
                node_num = self.f_send_cpu[layer][i].shape[0]
                self.f_send_cpu[layer][i].resize_([node_num+1, num_embed_send])
                node_num = self.f_recv_cpu[layer][i].shape[0]
                self.f_recv_cpu[layer][i].resize_([node_num+1, num_embed_send])
                node_num = self.f_recv_gpu[layer][i].shape[0]
                self.f_recv_gpu[layer][i].resize_([node_num+1, num_embed_send])

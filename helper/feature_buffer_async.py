...
class Buffer(object):
    ...
    def update(self, layer, feat):
        ...
        if self._use_async is False:
            ...
            if self._epoch > 0:
                with comm_timer.timer(f'epoch:{self._epoch} forward_{layer}'):
                    self._f_cpu_event[layer].wait() # wait last eopch forward finish : self._f_cpu_event[layer].set()
                    torch.cuda.current_stream().wait_event(self._f_cuda_event[layer])
                    self._f_cpu_event[layer].clear()  # Event.wait() will block
            self._f_buf[layer] = self.__feat_concat(layer, feat)
    ...
    def __gloo_all_to_all(self, send_gpu, send_cpu, recv_cpu, recv_gpu, tag, avg=None, forward=True):
        rank, size = dist.get_rank(), dist.get_world_size()
    ...
    def __feat_transfer(self, epoch, layer, feat):
        ...
        if self._backend == 'gloo':
            self.__gloo_all_to_all(feat, self._feat_cpu[layer], self._f_recv_cpu[layer], self._f_recv[layer],
                                   tag, self._f_avg[layer], forward=True)
            self._f_cuda_event[layer].record(self._comm_stream)
        else:
            raise NotImplementedError
        self._f_cpu_event[layer].set()  # Event.wait() will not block
    ...
    def __grad_hook(self, epoch, layer):
        def fn(grad):
            ...
            if self._use_async is False:
                ...
                if self._epoch > 0:
                    with comm_timer.timer(f'epoch:{self._epoch} backward_{layer}'):
                        self._b_cpu_event[layer].wait()   # wait last eopch backward finish : self._b_cpu_event[layer].set()
                        torch.cuda.current_stream().wait_event(self._b_cuda_event[layer])
                        self._b_cpu_event[layer].clear()  # Event.wait() will not block
                self.__update_grad(layer, grad)
    ...
    def __grad_transfer(self, epoch, layer, grad):
        ...
        if self._backend == 'gloo':
            self.__gloo_all_to_all(grad, self._grad_cpu[layer], self._b_recv_cpu[layer], self._b_recv[layer],
                                   tag, self._b_avg[layer], forward=False)
            self._b_cuda_event[layer].record(self._comm_stream)
        else:
            raise NotImplementedError
        self._b_cpu_event[layer].set()  # Event.wait() will not block
